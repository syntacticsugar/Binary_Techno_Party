...
class Game
  ...
  def initialize
    ...
    move_player1
    # move_somebody(@player1)
  end

  def somebody_fucking_won?
    @b.winner?(@player1) or @b.winner?(@player2)
  end

  def nobody_fucking_won?
    #!somebody_fucking_won #lazy shortcut, but I want to practice the logic
    !@b.winner?(@player1) and !@b.winner?(@player2)
  end

  def move_somebody(player)
    queue = [player,@player2]
    queue = ['X', 'O']
    if nobody_fucking_won?
      puts "Alright, #{player}. Which cell do you choose?"
      choice = gets.chomp.downcase
      if nobody_fucking_won?
        queue.reverse # move_somebody(player)
      end
    elsif somebody_fucking_won?
      game_over
    else
    end
  end
...
=end
      ...
      @choice = gets.chomp.to_s.downcase #to_s unnecessary
